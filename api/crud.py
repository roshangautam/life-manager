...
def get_transactions_by_household(db: Session, household_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Transaction).filter(models.Transaction.household_id == household_id).offset(skip).limit(limit).all()
