...
def generate_proto_code(proto_file: Path):
    ...
    try:
        ...
        print(f"  → Successfully generated code in {output_subdir.relative_to(PROJECT_ROOT)}")
        ...
        try:
            with open(py_file, 'r+', encoding='utf-8') as f:
                ...
                content = content.replace(
                    'import api.v1',
                    'from api.generated.api import v1 as api__v1'
                )
                content = content.replace(
                    'from api.v1',
                    'from api.generated.api.v1'
                )
                content = content.replace(
                    'import google',
                    'from api.generated import google'
                )
            ...
            print(f"  → Error fixing imports in {py_file.relative_to(PROJECT_ROOT)}: {e}", file=sys.stderr)
    ...
    for dirpath, _, _ in os.walk(OUTPUT_DIR):
        ...
        if not os.path.exists(init_file):
            with open(init_file, 'w', encoding='utf-8') as f:
                ...
                f.write("\n")
                f.write(f'"""Auto-generated package for {os.path.basename(dirpath)}."""\n')
                f.write("\n")
                f.write("__all__ = []\n")
...
def main():
    ...
    print(f"Generating gRPC code from .proto files in {PROTO_DIR.relative_to(PROJECT_ROOT)}...")
